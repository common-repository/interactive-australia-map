...
function freeaustralia_html5map_plugin_scripts(){
    if ( is_admin() ){
        ...
        wp_register_style('freeaustralia-html5-map-adm', plugins_url('/static/css/mapadm.css', __FILE__), array(), '3.4.8');
        wp_enqueue_style('freeaustralia-html5-map-adm');
        wp_register_style('freeaustralia-html5-map-style', plugins_url('/static/css/map.css', __FILE__), array(), '3.4.8');
...
function freeaustralia_html5map_plugin_content($atts, $content) {
    ...
    wp_register_script('raphaeljs', freeaustralia_html5map_plugin_get_raphael_js_url(), array(), '3.4.8');
    wp_register_script('freeaustralia-html5-map-mapjs_'.$type_id, freeaustralia_html5map_plugin_get_map_js_url($options), array('raphaeljs'), '3.4.8');
    ...
    if ($options['useAjaxUrls']) {
        ...
        $stateInfoUrl = "{$siteURL}/' + 'index.php' + '?map_id={$map_id}' + '&freeaustraliahtml5map_get_state_info="; // splitting urls in attempt to prevent this links from beeing parsed by google
        $groupInfoUrl = "{$siteURL}/' + 'index.php' + '?map_id={$map_id}' + '&freeaustraliahtml5map_get_group_info="; // splitting urls in attempt to prevent this links from beeing parsed by google
    }
    ...
    if ($options['areasList']) {
        ...
        wp_register_script('freeaustralia-html5-map-nicescroll', plugins_url($niceScrollJsFile, __FILE__), array(), '3.7.2'); // version from jquery.nicescroll.js
    ...
    $addInfoContainerSelector = "#freeaustralia-html5-map-state-info_{$count}";
    ...
    $mapJs =  "<script type=\"text/javascript\">
            jQuery(function(){
                ...
                if (link == '#info') {
                    ...
                    jQuery('{$addInfoContainerSelector}').html('". __('Loading...', 'freeaustralia-html5-map') ."');
                    jQuery.ajax({
                        type: 'POST',
...
function freeaustralia_html5map_plugin_delete_options($blog_id = null, $option_name = 'freeaustraliahtml5map_options') {
    if ( is_multisite() ) {
...
function freeaustralia_html5map_plugin_activation() {
    ...
    if (is_multisite()) {
        add_blog_option(get_current_blog_id(), 'freeaustraliahtml5map_options', $options);
    } else {
...
function freeaustralia_html5map_plugin_uninstall() {
    if (is_multisite()) {
        if (function_exists('get_sites')) {
            $res = get_sites();
            foreach ($res as $blog) {
                delete_blog_option($blog->blog_id, 'freeaustraliahtml5map_options');
                delete_blog_option($blog->blog_id, 'freeaustraliahtml5map_goptions');
            }
        }
    } else {
        delete_site_option('freeaustraliahtml5map_options');
        delete_site_option('freeaustraliahtml5map_goptions');
    }
}
...
function freeaustralia_html5map_plugin_import(&$errors) {
    ...
    if(is_uploaded_file($_FILES['import_file']["tmp_name"])) {
        ...
        if ($data) {
            ...
            $options = freeaustralia_html5map_plugin_get_options();
            $defaultCfg = freeaustralia_html5map_plugin_map_defaults('', 1, true);
                ...
                if (isset($map_data['map_data']) and $map_data['map_data']) {
                    ...
                    foreach ($data as $stId => $datum) {
                        if (!isset($def_settings[$stId])) {
                            $errors[] = sprintf(__('Section "%s" skipped cause "map_data" does not belong to this map.', 'freeaustralia-html5-map'), $map_id);
                            continue 2;
                        }
